...
def run(settings):
    ...
    dataset_train = sampler.HCATSampler([lasot_train, got10k_train, coco_train, trackingnet_train], [1,1,1,1],
                                samples_per_epoch=600, max_gap=100, processing=data_processing_train)
    ...
    qat_model = torch.nn.parallel.DistributedDataParallel(qat_model, device_ids=[settings.local_rank], find_unused_parameters=True)
    model_without_ddp = qat_model.module
    ...
    trainer.train(20, load_latest=False, fail_safe=True)
