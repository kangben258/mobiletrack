...
class MobileOne(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.stage3(x)
        # x = self.stage4(x)
        self._add_output_and_check('layer3', x, outputs, output_layers)
        # x = self.gap(x)
        # x = x.view(x.size(0), -1)
        # x = self.linear(x)
        return outputs
...
def mobileone_s0(output_layers=None, pretrained=False, frozen_layers=None):
    model = mobileone(num_classes=1,inference_mode=False,pretrained=True,variant="s0")
    return model

def mobileone(num_classes: int = 1000, inference_mode: bool = False, pretrained = False,
    ...
    weights = "/home/kb/HiT/checkpoints/mobileone_s0_unfused.pth.tar"
