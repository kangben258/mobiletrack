...
class Tracking:
    def __init__(self,size,convert_type):
        self.size = size
    ...
    def __call__(self, img,bbox):
        ...
        if self.type == "backbone":
            ...
            z_crop = self.get_subwindow(img, center_pos,
                                        self.size,
                                        s_z, channel_average)
            mean_ = torch.Tensor([0.485, 0.456, 0.406]).view(1, -1, 1, 1).cuda()
            std_ = torch.Tensor([0.229, 0.224, 0.225]).view(1, -1, 1, 1).cuda()
            z_crop /= 255
            z_crop -= mean_
            z_crop /= std_
        elif self.type == "complete":
            w_z = size[0] + (4 - 1) * ((size[0] + size[1]) * 0.5)
            h_z = size[1] + (4 - 1) * ((size[0] + size[1]) * 0.5)
            s_z = math.ceil(math.sqrt(w_z * h_z))
            channel_average = np.mean(img, axis=(0, 1))
            z_crop = self.get_subwindow(img, center_pos,
                                        self.size,
                                        round(s_z), channel_average)
            mean_ = torch.Tensor([0.485, 0.456, 0.406]).view(1, -1, 1, 1).cuda()
            std_ = torch.Tensor([0.229, 0.224, 0.225]).view(1, -1, 1, 1).cuda()
            z_crop /= 255
            z_crop -= mean_
    ...
    def get_subwindow(self, im, pos, model_sz, original_sz, avg_chans):
        ...
        im_patch_context = cv.resize(im_context, (c_context_resize, r_context_resize))
